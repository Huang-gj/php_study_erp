/*
 * ERP 采购模块数据库设计（MySQL 8.x）
 * 设计说明：
 * 1. 本文件是基于你前面的销售模块继续扩展出来的“采购 + 供应商 + 进项开票 + 请购”设计。
 * 2. 物料主数据建议复用 sales_product；如果后续你要做最佳实践重构，建议把 sales_product 下沉为 base_item。
 * 3. 供应商统计页、开票汇总页优先由核心业务表实时统计，不强制单独建报表表。
 * 4. 因截图里已出现“到货数量”“已付款金额”等字段，因此底层补了收货单、付款单两组表，避免只有汇总值没有业务痕迹。
 * 5. 所有表继续沿用你的字段习惯：id / del_state / del_time / create_time。
 */

...
/*
 * ERP 销售模块数据库设计（MySQL 8.x）
 * 设计说明：
 * 1. 按“核心业务表 + 报表汇总表”设计，不按“一个页面一张表”去硬拆。
 * 2. 所有列表、分析页、统计页尽量由核心表实时查询；月报/日报量大时再落汇总表。
 * 3. 保留你常用的基础字段风格：id / del_state / del_time / create_time。
 * 4. 金额统一单位：元；数量统一保留 4 位小数；单价统一保留 4 位小数。
 * 5. 本设计默认 user 表已存在，user_id 对应你系统里的用户表。
 */
